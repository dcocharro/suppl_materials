...
function getSortedByDistance() {
    ...
    rhythmicWorld.forEach((obj, index) => {
        post(`${index}. Distance: ${obj.dist2user.toFixed(3)}  - Pattern: ${JSON.stringify(obj.rhythm)}  - Meter: ${JSON.stringify(obj.meter)}\n`);
    });
...
function getNearestNeighbors(n = 5) {
    ...
    neighbors.forEach((obj, i) => {
        post(`${i}. Distance: ${obj.dist2user.toFixed(3)}  - Pattern: ${JSON.stringify(obj.rhythm)}  - Meter: ${JSON.stringify(obj.meter)} \n`);
    });
